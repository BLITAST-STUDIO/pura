...
export default function FusionLab() {
  ...
  const [preset, setPreset] = useState<FusionPreset>('pair');
  const [ratio, setRatio] = useState(1);
  const [options, setOptions] = useState<FusionOptions>({ lighting: 'studio', inspection: false, paused: false, reducedMotion: window.matchMedia('(prefers-reduced-motion: reduce)').matches, quality: 'high', clay: false });
