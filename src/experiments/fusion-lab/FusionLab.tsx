...
export default function FusionLab() {
  ...
  const [options, setOptions] = useState<FusionOptions>({ lighting: 'studio', inspection: false, paused: false, reducedMotion: window.matchMedia('(prefers-reduced-motion: reduce)').matches, quality: 'high', clay: false, dyeFlow: initialDyeFlow(), ripple: initialRipple(), caustic: initialCaustic() });
    ...
    <main>
      ...
      <div className="dl-controls">
        ...
        {settings && <div className="dl-settings-panel" id="fusion-settings" role="dialog" aria-label="表示と動きの設定"><div className="dl-settings-heading"><span>表示と動き</span><button ref={closeButton} className="dl-icon-button" aria-label="設定を閉じる" onClick={() => { setSettings(false); settingsButton.current?.focus(); }}><X size={15}/></button></div>
          ...
          <label className="dl-setting-row"><span>縁が波打つ</span><input type="checkbox" checked={!!options.ripple} onChange={e => { change({ ripple: e.target.checked }); writeLookQuery('ripple', rippleQuery(e.target.checked)); }}/><span className="dl-switch"/></label>
          <label className="dl-setting-row"><span>形から床の光を描く</span><input type="checkbox" checked={options.caustic === 'shape'} onChange={e => {
            change({ caustic: e.target.checked ? 'shape' : 'artistic' });
