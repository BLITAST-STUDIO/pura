...
import { BENCH_SCENARIOS, BENCH_TIMING, summarize, type BenchResult } from './measure';
...
export default function BenchPlay() {
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const [index, scenario] of BENCH_SCENARIOS.entries()) {
        ...
        experience.setOptions({ quality: scenario.quality as FusionOptions['quality'], lighting: 'studio', dyeFlow: 'bloom', paused: false, reducedMotion: false,
        ...
        setPhase(`${index + 1}/${BENCH_SCENARIOS.length} · ${scenario.count}個 · ${scenario.quality === 'high' ? '美しさ優先' : '軽さ優先'} · 慣らし`);
        await wait(BENCH_TIMING.warmupMs);
        measuring = true;
        setPhase(`${index + 1}/${BENCH_SCENARIOS.length} · ${scenario.count}個 · ${scenario.quality === 'high' ? '美しさ優先' : '軽さ優先'} · 計測中`);
  ...
  return <div className="droplet-lab purity-scene stage-play bench-play" data-lighting="studio" data-hue="cyan"><div className="dl-shell">
    ...
    <main>
      <p className="stage-title"><b>計測</b>このまま画面を開いておいてください。約{Math.round(BENCH_SCENARIOS.length * (BENCH_TIMING.warmupMs + BENCH_TIMING.measureMs) / 1000 / 5) * 5}秒で終わります。触らなくて大丈夫です。</p>
      ...
      <table className="bench-table"><thead><tr><th>個数</th><th>画質</th><th>fps</th><th>中央値</th><th>p95</th><th>50ms超</th></tr></thead>
        <tbody>{results.map(r => <tr key={`${r.count}-${r.quality}`} className={r.medianMs > 20 ? 'is-slow' : ''}><td>{r.count}</td><td>{r.quality === 'high' ? '美しさ' : '軽さ'}</td><td>{r.fps}</td><td>{r.medianMs} ms</td><td>{r.p95Ms} ms</td><td>{r.over50}</td></tr>)}</tbody></table>
