...
## 8ステージのテストプレイ（2026-09-25）

- ソース `c8140a5`、配信 `9b5634d`。試遊 `?play=stages`。全15ファイルSHA-256一致。公開版で第1・4・6面とサンドボックス60個を表示・操作し警告エラー0。

## スコアアタック（2026-09-25）

- ソース `1d54baf`、配信 `7372640`。`?play=stages&mode=score`。全ファイルSHA-256一致。公開版でコンボ×9・クリア4386点を確認、警告エラー0。
...
## 連続融合の改善と計測ページ（2026-09-26）

- ソース `eddb41d`、配信 `51a75e6`。`?play=bench` を追加。全25ファイルSHA-256一致。公開版で計測ページが66秒で6条件を完了（表示まで1352 ms、Mac M1）、通常URL・元祖の表示を確認、警告エラー0。

## 自動の解像度調整（2026-09-26）

- ソース `a1ac191`、配信 `14cb985`。全ファイルSHA-256一致。公開版の通常URL・モード移動で警告エラー0。
- ソース `7eccf62`、配信 `1caa05a`（計測ページに倍率の内訳）。SHA-256一致。

## 見た目の3案（2026-09-26）

- ソース `b1ee819`（ブランチ `design/direction`）、配信 `ea46083`。`?look=night` / `?look=gallery`。全ファイルSHA-256一致。公開版でステージ・自由の盤面が3:4（スマホ348×465、PC幅418×558）になったことを確認。

## 白い画面（2026-09-26）

- ソース `c381164`（ブランチ `design/direction`）、配信 `3e361f7`。`?ui=light` と `?look=` の組み合わせ。全25ファイルSHA-256一致。公開版で白い画面×3つの盤面（入口プレイ・ステージ3）を開き、警告エラー0。

## 白い画面×白い展示室を既定に（2026-09-26）

- ソース `ae39472`、配信 `4fe0319`。全25ファイルSHA-256一致。公開版で通常URL・ステージ・自由・三つの道が白い画面×白い展示室、`?ui=dark&look=studio` と元祖は暗いまま、警告エラー0。

## ひとふで と 道・第3章の試作（2026-09-26）

- ソース `8969c72`（ブランチ `feat/hitofude`）、配信 `bcad18d`。全26ファイルSHA-256一致。公開版で、ひとふで面1を引いて離す操作で1手クリア（★3）、3-1で2回タップの分離（雫6→7）と約25秒後の一文を確認、警告エラー0。

## ひとふで 9ホールのコース（2026-09-26）

- ソース `7c3e9f8`（ブランチ `feat/hitofude-course`）、配信 `c72b0df`。全26ファイルSHA-256一致。公開版で9番を1打（「ひとふで −2」）、ラウンドで1・2番を1打ずつ回りスコアカードに記録されることを確認、警告エラー0。

## 道（四つの章）（2026-09-27）

- ソース `8bdae9f`（ブランチ `feat/michi`）、配信 `1eb02b7`。全29ファイルSHA-256一致。公開版で道の16面すべてを開き雫・輪・目標表示が想定どおり、モード切り替えが5つ、縦長PC画面でステージ系の枠が560pxになったことを確認、警告エラー0。

## カーリング（2026-09-27）

- ソース `5a8f9f4`（ブランチ `feat/curling`、道を含む）、配信 `675c3c3`。全32ファイルSHA-256一致。公開版でCPU戦の1エンドを通し（CPUの先読みと投球、得点、スコアボード）、警告エラー0。

## 壁を見えるように（試作、2026-09-27）

- ソース `49d85e3`（ブランチ `feat/walls`）、配信 `cf7f65d`。全32ファイルSHA-256一致。公開版で、ひとふで・カーリング・はじめるの3画面を縁／線／なしで表示し、警告エラー0。

## 縁を既定に（main、2026-09-27）

- ソース `39e0f42d477ad07fb37f45323313b231aa769e99`（`main`、PR #6 統合後）、配信 `c713bec`。全32ファイルSHA-256一致。

## 仕上げ（2026-09-27）

- ソース `a143af169c12b11a9972dd4332c85a8f2db09eb0`（ブランチ `feat/finish`）、配信 `8d9283f`。全33ファイルSHA-256一致。公開版で道1-1をクリアし光の輪→一拍おいて結果の順、スコア画面が8ステージのみ、警告エラー0。

## ひとふで18ホールと挑戦状（2026-09-27）

- ソース `69826eaaaf524bbbe13894a37de70e42624b599d`（ブランチ `feat/hitofude-back9`）、配信 `7ee001e`。全33ファイルSHA-256一致。公開版で18番を1打で決め（ひとふで）、警告エラー0。

## 今日のホール（2026-09-27）

- ソース `1aab8a685022f888fef3c1ee6cb8e8b49f64a657`（ブランチ `feat/hitofude-back9`）、配信 `90e2d33`。全33ファイルSHA-256一致。公開版で今日のホールを一打で決め（ひとふで −1）、警告エラー0。
